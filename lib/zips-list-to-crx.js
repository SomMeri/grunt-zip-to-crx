module.exports = function() {
  ...
  this.thickThing = function (options, allSrcZips, crxDestination, originalSrcZip, originalCrxDestination, onDone, onError) {
    ...
    async.each(allSrcZips,function(srcPath, callback){
      ...
      crxCreator.generateCrxFile(srcPath, fullDestination, options.privateKey, callback);
    }, function(err){ // when all are done here
       if (err){
         onError(err);
       } else {
         onDone();
